...
describe("LiarsLobby", function () {
  async function deployFixture() {
    ...
    await liarsLobby.initialize([], liarsToken.address);

    return { liarsLobby, liarsToken, owner, player1, player2, player3, others };
  }
  ...
  describe("Initialization", function () {
    it("should initialize with correct token address", async function () {
      ...
      expect(await liarsLobby.liarsToken()).to.equal(await liarsToken.getAddress());
    });

    it("should initialize with empty player list", async function () {
      // TODO: Vérifier que la liste des joueurs est vide à l'initialisation.
    });

    it("should initialize in Waiting state", async function () {
      // TODO: Vérifier que l'état initial est bien 'Waiting' (ou l'équivalent).
    });

    it("should initialize with current turn index at 0", async function () {
      // TODO: Vérifier que l'indice du tour courant est à 0.
    });
  });
